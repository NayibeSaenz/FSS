    ...
    class Users { 
		...
		public function index(){
			...
			if (isset($_SESSION['usuario'])) {
				...
				require_once 'views/modules/0_mains/'.$this->modulo.'.main.view.php';
        ...
        public function crear(){
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'GET'){
                    require_once 'views/roles/'.$this->modulo.'/header.php';
                    require_once 'views/modules/1_users/0_create_users.view.php';
                    require_once 'views/roles/'.$this->modulo.'/footer.php';
                }
                if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                    $usuario = new User();
                    $usuario->setDocUsuario($_POST['documento']);
                    $usuario->setTipoDocUsuario($_POST['tipodoc']);
                    $usuario->setNombresUsuario($_POST['nombres']);
                    $usuario->setApellidosUsuario($_POST['apellidos']);
                    $usuario->setTelUsuario($_POST['telefono']);
                    $usuario->setCelUsuario($_POST['celular']);
                    $usuario->setEmailUsuario($_POST['email']);
                    $usuario->setDirUsuario($_POST['direccion']);
        ...
        public function consultar(){
                ...
                require_once 'views/roles/'.$this->modulo.'/header.php';
                require_once 'views/modules/1_users/1_read_users.view.php';
                require_once 'views/roles/'.$this->modulo.'/footer.php';
            } else {
                header('Location: ?c=Dashboard');
            }	
        ...
        public function consultarClientes(){
                ...
                require_once 'views/roles/'.$this->modulo.'/header.php';
                require_once 'views/modules/2_mod_main/1_crud_clientes/1_read_clientes.view.php';
                require_once 'views/roles/'.$this->modulo.'/footer.php';
            } else {
                header('Location: ?c=Dashboard');
            }	
        ...
        public function consultarEntrenadores(){
            $valUsuario = unserialize($_SESSION['usuario']);
            if (isset($_SESSION['usuario']) && ($valUsuario->getIdRol() == 1 )) {
            $entrenadores = $this->model->listarEntrenadores();
            require_once 'views/roles/'.$this->modulo.'/header.php';
            require_once 'views/modules/2_mod_main/2_crud_entrenadores/1_read_entrenadores.view.php';
            require_once 'views/roles/'.$this->modulo.'/footer.php';
        } else {
            header('Location: ?c=Dashboard');
        }	
        ...
        if (isset($_SESSION['usuario']) && ($valUsuario->getIdRol() == 1 )) {
            # POR GET
            if ($_SERVER['REQUEST_METHOD'] == 'GET') {
                $user = $this->model->getById($_GET['documento']);
                $tipodoc = ['CC', 'TI', 'CE'];
                $perfil = ['Admin', 'Entrenador', 'Cliente'];
        ...
        public function eliminar(){
            $valUsuario = unserialize($_SESSION['usuario']);
            if (isset($_SESSION['usuario']) && $valUsuario->getIdRol() == 1) {
                $this->model->eliminar($_GET['documento']);
                header('Location: ?c=Users&a=consultar');
            } else {
                header('Location: ?c=Dashboard');
            }
